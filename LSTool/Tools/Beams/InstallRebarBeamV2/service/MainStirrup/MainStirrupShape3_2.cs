...
{
    public class MainStirrupShape3_2(MainStirrupCollectionDto mainStirrupDto) : InstallMainStirrupRebarBeam(mainStirrupDto)
    {
        protected override void PlaceRebar(RectangleDto rectangleDto, int chanLe)
        {
            try
            {
...

                {
                    ...
                    var rebar = Rebar.CreateFromRebarShape(MainStirrupDto.Document, SecondaryShapeStirrup.GetRebarShape135_135(),
                        MainStirrupDto.RebarBarTypeCustom.RebarBarType, MainStirrupDto.Host
                        , origin, xVec, yVec);
                    rebar.RebarScaleToBox(origin, xVec, yVec);
                    rebar.SetSolidRebar3DView(MainStirrupDto.Document.ActiveView);
            ...
            {
                throw new InvalidOperationException("Failed to create main stirrup shape 3-2.", ex);
            }
        }
    }
}
